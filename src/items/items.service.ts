...
export class ItemsService {
    ...
    async findAll(options: {
        ...
        if (sortField === 'price' || filterField === 'price') {
            const priceFilters: PipelineStage[] = [
                {
                    $project: {
                        name: 1,
                        priceWithDiscount: {
            ...
            const items = await this.itemModel.aggregate(priceFilters);

            const itemsNoPaging = await this.itemModel
            .countDocuments(filter);

            return { items, totalCount: itemsNoPaging };
        }
